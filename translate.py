...
import hmac
import json
from audio_interface import get_random_audio_file, speech2text, convert_mp3_to_wav, audio_sample
        ...
        if self.APPID == '' or self.APIKey == '' or self.Secret == '':
            ...
            if status_code != 200:
                ...
                if str(respData["code"]) != '0':
                    print("请前往https://www.xfyun.cn/document/error-code?code=" + str(respData["code"]) + "查询解决办法")
                else:
                    return respData["data"]["result"]["trans_result"]["dst"]

def translate_yue_to_cn(text):
